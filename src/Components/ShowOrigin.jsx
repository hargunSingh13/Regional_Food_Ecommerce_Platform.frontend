...
const ShowOrigin = () => {
    ...
    useEffect(() => {
        if (ori && products.length > 0) {
            setFilterData(products.filter((product) => product.origin.toLowerCase() === ori.toLowerCase()))
        }
    }, [ori, products, setFilterData])
    return (
        <>
            <Headder />

            <div className='p-5'>
                <h1 className='text-center text-purple text-3xl font-bold mb-6 pb-5'>{`Explore FOOD in ${ori.toUpperCase()}`}</h1>
